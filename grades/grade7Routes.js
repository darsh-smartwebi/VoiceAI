...
function loadPdfTableOnce() {
  const csv = fs.readFileSync("./csv/grade7.csv", "utf8");
  ...
  console.log(`✅ Loaded ${PDF_TABLE.length} PDFs from grade7.csv`);
}
...
function scoreMatch(search, candidate) {
  ...
  const important = [
    ...
    // Combined Forms (important for matching full titles)
